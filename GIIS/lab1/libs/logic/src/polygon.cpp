    ...
    do {
        ...
        for (size_t i = 0; i < points.size(); i++) {
            ...
            if (cross < 0) {
                ...
                int dist_to_candidate = (p3.first - p1.first)*(p3.first - p1.first) + 
...
point_vector graham_scan(const point_vector &points) {
  ...
  std::sort(sorted_points.begin() + 1, sorted_points.end(),
            [&p0](const std::pair<int, int> &a, const std::pair<int, int> &b) {
              double angle_a =
                  calculate_polar_angle(p0.first, p0.second, a.first, a.second);
              double angle_b =
                  calculate_polar_angle(p0.first, p0.second, b.first, b.second);

              if (angle_a == angle_b) {
                int dist_a = (a.first - p0.first) * (a.first - p0.first) +
                             (a.second - p0.second) * (a.second - p0.second);
                int dist_b = (b.first - p0.first) * (b.first - p0.first) +
...
color_point_vector draw_convex_polygon(point_vector vertices) {
  ...
  for (size_t i = 0; i < vertices.size(); i++) {
    ...
    std::vector<std::tuple<int, int, double>> line_points =
        draw_CDA(current.first, current.second, next.first, next.second);

    all_points.insert(all_points.end(), line_points.begin(), line_points.end());
  }

  return all_points;
}

bool is_point_inside_polygon(const point_vector& point, const point_vector& polygon) {
    if (point.size() != 1 || polygon.size() < 3) return false;
...
color_point_vector get_line_polygon_intersections(const point_vector& line, const point_vector& polygon) {
    ...
    if (line.size() != 2 || polygon.size() < 3) return intersections;
