...
bool is_on_polygon_boundary(int x, int y, const point_vector &polygon) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (dot < 0 || dot > (x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1))
      continue;
...
color_point_vector fill_polygon_scanline_active(const point_vector &polygon) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (is_local_min) {
      ...
      edge_table[y_min].push_back(ActiveEdge(x_start, dx, y_max));
    } else {
  ...
  while (!seed_stack.empty()) {
    ...
    for (int fill_x = x_left + 1; fill_x < x_right; fill_x++) {
      if (!filled_set.count({fill_x, y}) &&
        ...
        filled_pixels.push_back(std::make_tuple(fill_x, y, 0));
      }
    }
    ...
    if (check_y >= min_y) {
      ...
      for (int check_x = x_left; check_x <= x_right; check_x++) {
        ...
        bool is_in_polygon = is_point_in_polygon(check_x, check_y, polygon) ||
                             is_on_polygon_boundary(check_x, check_y, polygon);
        bool is_filled = filled_set.count({check_x, check_y});
        ...
        if (is_in_polygon && !is_filled) {
          ...
          if (in_span) {
            ...
            int seed_x_new = (span_start + check_x - 1) / 2;
            if (!filled_set.count({seed_x_new, check_y})) {
              seed_stack.push({seed_x_new, check_y});
            }
            in_span = false;
          }
        }
